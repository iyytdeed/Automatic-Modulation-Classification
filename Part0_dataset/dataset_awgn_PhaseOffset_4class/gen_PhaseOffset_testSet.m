...
NClass = 4;
j = sqrt(-1);
phase_offset_array = PhaseOffset_low:gap:PhaseOffset_high;
test_data = zeros(length(phase_offset_array) * NClass * NTest, L + 2);
%% get test data
for idx = 1:length(phase_offset_array)
    PhaseOffset = phase_offset_array(idx)*(pi/180);
    fprintf("PhaseOffset = %f\n", phase_offset_array(idx));
    fprintf("PhaseOffset = %f\n", PhaseOffset);
    % bpsk
    ...
    for row = 1:NTest
        ...
        signal_data(row, L + 2) = phase_offset_array(idx);
    end
    for row = 1:NTest
...
save('./dataset/test_data_PhaseOffset.mat', 'test_data', '-mat');
