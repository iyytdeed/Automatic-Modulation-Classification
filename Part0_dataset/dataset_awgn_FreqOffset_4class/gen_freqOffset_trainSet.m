...
EsNoHigh = 20;
FeqOffset_low = 0;
FeqOffset_high = 0.5*pi/L/2;
%FeqOffset_high = 0.5*pi/L; % normalize to maximun rotation of 90 degree
NClass = 4;
...
for row = 1:NTrain
    ...
    P = 10^(EsNo/10);
    freq_offset = FeqOffset_high*rand; % freq offset ~ U(0, FeqOffset_high)
    for col = 1:L
        s = x(unidrnd(xN));
...
for row = 1:NTrain
    ...
    P = 10^(EsNo/10);
    freq_offset = FeqOffset_high*rand;
    for col = 1:L
        s = x(unidrnd(xN));
        signal_data(row, col) = sqrt(P)*exp(j*(freq_offset*col))*s + sqrt(1/2)*(randn+j*randn);
    end
