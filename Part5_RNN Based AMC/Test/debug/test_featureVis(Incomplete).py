...
def draw_signal_scale(data, row, col, n):
    ...
    i_array = data[0,:]
    q_array = data[1,:]
    plt.scatter(x_array, i_array)
    plt.scatter(x_array, q_array)
...
first_layer = first_layer.reshape((first_layer.shape[2], first_layer.shape[3]))
...
for row in range(8):
    for col in range(8):
        n = 8*row+col+1
        plt.subplot(8, 8, n)
        x_array = range(first_layer.shape[0])
...
for row in range(8):
    for col in range(8):
        n = 8*row+col+1
        plt.subplot(8, 8, n)
        x_array = np.zeros(first_layer.shape[0])
        feature_array = first_layer[:,n-1]
