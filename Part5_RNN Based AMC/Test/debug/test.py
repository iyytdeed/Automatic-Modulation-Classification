...
# load model
model = model_from_json(json.load(open("lstm_model_struct.json")))

# load wights
model.load_weights("./lstm_model_struct.h5")
model.summary()
...
data_real = data_real.reshape((data_real.shape[0], seqLen))
data_imag = data_imag.reshape((data_imag.shape[0], seqLen))
x_test = np.stack((data_real, data_imag), axis=2)
