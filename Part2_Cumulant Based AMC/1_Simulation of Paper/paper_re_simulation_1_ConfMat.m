...
FreqOffsetBound = 15/L
FreqOffsetBound = FreqOffsetBound*pi/180;
...
xN = length(x);
for row = 1:N
    freq_offset = 2*FreqOffsetBound*rand() - FreqOffsetBound;
    for col = 1:L
        s = x(unidrnd(xN));
        phase_jitter = 2*PhaseBound*rand() - PhaseBound;
        signalData(row, col) = sqrt(P)*exp(j*(freq_offset*col) + j*phase_jitter)*s +...
            sqrt(1/2)*(randn+j*randn);
